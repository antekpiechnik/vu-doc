...
programs (both shows and documentaries), movie actors and TV actors from the
freebase.com archive. Those titles will be used to analyze and match the
...
\\ Occurrences of entity names a few times in a stream without a
...
Twitter users also use Hashtags for easier retrieval of tweets mentioning
...
\end{itemize}
\subsubsection{Following preference entities}
...
\subsection{Types of messages - results}
\subsubsection{Diverting results}
In order for the results to actually bring any outcome, we decided to divide the
corpus into two halves. By measuring the
difference in achieved results, we might be actually able to evaluate their
usefulness.
...
In order to confirm that statement, we have attempted to match topics related to those entities in their followers streams. By selecting attributes
describing their relation to the topic (such as show names actors starred in). The problem here is some of the topics are a bit too general (such as \textit{Writer}
as a TV-related job). Finding a way of ranking those categories could provide us with reducing the noise in those measurements.

This leads to a conclusion that following a certain username most definitely states users interest in a notable part of topics related to that entity and
...
This value might also be modified by the among of different entities found,
...
In order to reduce the chance of such noise occurring, we decided to run those entity names against a corpus of English texts, ranking them
by their popularity. The more often an entity name occurs in those corpora, the lesser the chance of it occurring as a name of the TV shows.

In order to measure the certainty, a separate function has been introduced, taking into account the following:
...
This measurement allowed us to rank some of the entity names completely unusable (such as show names like \textit{'Me too'} or \textit{'Trust me'}, which popularities make their
occurrences unusable unless e.g. they occur in a form of a \textit{Hashtag}). The result of certainty will be easily translatable into the preference weights to be recorded when profiling a user.
