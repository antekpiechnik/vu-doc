...
\subsubsection{Conclusions from counting YouTube tags}
...
candidates for such a set. Also, user's uploaded videos have been considered and a
...
small amount of false positives. We might still try to improve these results.

\subsection{Attempt at improving tags aggregation}

Even though the tags gathered across clips from user's subscriptions give
relatively good overview on his interests, they give preference to channels with
larger number of videos. Specifically, a subscription on a very narrow topic,
which tags almost every video with a core set of keywords, would boost these
tags' score in this method on counting. To prevent that, we tried to vary the
weight of tags based on how many different keywords a single subscription has.
Unfortunately, results of this method were much worse than simple equal-weight
counting. Tags from single video channels gained enormous coefficients, which
caused them to jump right ahead of all the others. An extreme example of that
was a channel with a single video marked with a single keyword. It received a
factor of 1, which made it impossible for any other tag to get a higher ranking
position, while obviously -- this single word did not express original user's
interests.
