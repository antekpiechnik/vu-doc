...
of user activity for both services. We expect many \textit{false positives} to occur, i.e. incorrectly identified NEs
...
\begin{figure}[h!]
  \begin{center}
    ...
    \end{tabular} \\
    Table 8: Rate of video descriptions with entities found using using full name matching. \\
  \end{center}
\end{figure}
...
We have measured the average ratio of tweets with entities found to all tweets for different kinds of entities
using full name matching of the entity name.
...
(around 4000 tweets), we were able to find out only 24 tweets (about 0.57\%) mentioning directly the people they are following. However, such little amount of data is not enough to base conclusions on.
...
that represent user's interests. Many YouTube videos have semantic meaning -- they can
...
each video has many categories and many tags applied. They are chosen from a limited
(12 items, for each video on \textit{YouTube}) set of options. Below is a comparison of
results of counting tags and categories.
...
information gathered. This is what makes tags much more useful for profile generation.
...
of mentions of different entities, giving a higher weight to more precise mentions, \eg
...
The results in Table 18 show that the analyzed \textit{YouTube} UAD contained more linked
